...
def create_recur(request):
    ...
    return render(request,'recurpage.html',{'item':item,'cus':cus})
...
def new_recur(request):
    if request.method=='POST':
        ...
        recur.save()
        # items=request.POST.getlist('iname[]')
        # quanti=request.POST.getlist('qua[]')
        # rates=request.POST.getlist('rate[]')
        # dis=request.POST.getlist('discount[]')
        # tax=request.POST.getlist('tax[]')
        # sub=request.POST.getlist('subtotal[]')
        # taxamt=request.POST.getlist('total_taxamount[]')
        # ship=request.POST.getlist('shipping_charge[]')
        # total=request.POST.getlist('total[]')
        # if len(items) == len(quanti) == len(rates)== len(dis)== len(tax)== len(sub)== len(taxamt)== len(ship)== len(total):
        #     mapped2 = zip(items,quanti,rates,dis,tax,sub,taxamt,ship,total)
        #     mapped2 = list(mapped2)
         
        #     for ele in mapped2:
        #         created = recur_itemtable.objects.get_or_create(iname=ele[0], quantity=ele[1],rate=ele[2],discount=ele[3],tax=ele[4],subtotal=ele[5],taxamt=ele[6],shipcharge=ele[7],total=ele[8])
...
def viewrecur(request,id):
    cust = customer.objects.filter(user_id=request.user.id).first()
...
def edit_recur(request,id):
    ...
    return render(request,'edit_recur.html',{'editr':editr,'cus':cus})
